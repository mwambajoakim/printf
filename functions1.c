#include "main.h"

/************************* PRINT UNSIGNED NUMBER *************************/

/**
 * print_unsigned - prints unsigned number
 * @types: arguments list
 * @buffer: buffer array to handle print
 * @flags: calculates active flags
 * @wid: get wid
 * @prec: prec specification
 * @size: size specifier
 *
 * Return: number of chars printed out
 */
int print_unsigned(va_list types, char buffer[],
                   int flags, int wid, int prec, int size)
{
    int w = BUFSIZ - 2;
    unsigned long int num = va_arg(types, unsigned long int);

    num = convert_size_unsgnd(num, size);

    if (num == 0)
        buffer[w--] = '0';

    buffer[BUFSIZ - 1] = '\0';

    while (num > 0)
    {
        buffer[w--] = (num % 10) + '0';
        num /= 10;
    }

    w++;

    return (write_unsgnd(0, w, buffer, flags, wid, prec, size));
}

/************************* PRINT UNSIGNED NUMBER IN OCTAL *************************/

/**
 * print_octal - prints an unsigned number in octal format
 * @types: arguments list
 * @buffer: buffer array to handle print
 * @flags: calculates active flags
 * @wid: get wid
 * @prec: prec specification
 * @size: size specifier
 *
 * Return: number of chars printed out
 */
int print_octal(va_list types, char buffer[],
                int flags, int wid, int prec, int size)
{
    int w = BUFSIZ - 2;
    unsigned long int num = va_arg(types, unsigned long int);
    unsigned long int init_num = num;

    UNUSED(wid);

    num = convert_size_unsgnd(num, size);

    if (num == 0)
        buffer[w--] = '0';

    buffer[BUFSIZ - 1] = '\0';

    while (num > 0)
    {
        buffer[w--] = (num % 8) + '0';
        num /= 8;
    }

    if (flags & F_HASH && init_num != 0)
        buffer[w--] = '0';

    w++;

    return (write_unsgnd(0, w, buffer, flags, wid, prec, size));
}

/************************* PRINT UNSIGNED NUMBER IN HEXADECIMAL *************************/

/**
 * print_hexadecimal - prints an unsigned number in hexadecimal format
 * @types: arguments list
 * @buffer: buffer array to handle print
 * @flags: calculates active flags
 * @wid: get wid
 * @prec: prec specification
 * @size: size specifier
 *
 * Return: number of chars printed out
 */
int print_hexadecimal(va_list types, char buffer[],
                      int flags, int wid, int prec, int size)
{
    return (print_hexa(types, "0123456789abcdef", buffer,
                       flags, 'x', wid, prec, size));
}

/************************* PRINT UNSIGNED NUMBER IN UPPER HEXADECIMAL *************************/

/**
 * print_hexa_upper - prints an unsigned number in upper hexadecimal format
 * @types: arguments list
 * @buffer: buffer array to handle print
 * @flags: calculates active flags
 * @wid: get wid
 * @prec: prec specification
 * @size: size specifier
 *
 * Return: number of chars printed out
 */
int print_hexa_upper(va_list types, char buffer[],
                     int flags, int wid, int prec, int size)
{
    return (print_hexa(types, "0123456789ABCDEF", buffer,
                       flags, 'X', wid, prec, size));
}

/************************* PRINT HEXA NUMBER IN LOWER OR UPPER *************************/

/**
 * print_hexa - prints a hexadecimal number in lower or upper
 * @types: arguments list
 * @map_to: array of values to map the number to
 * @buffer: buffer array to handle print
 * @flags: calculates active flags
 * @flag_ch: calculates active flags
 * @wid: get wid
 * @prec: prec specification
 * @size: size specifier
 *
 * Return: number of chars printed out
 */
int print_hexa(va_list types, char map_to[], char buffer[],
               int flags, char flag_ch, int wid, int prec, int size)
{
    int w = BUFSIZ - 2;
    unsigned long int num = va_arg(types, unsigned long int);
    unsigned long int init_num = num;

    UNUSED(wid);

    num = convert_size_unsgnd(num, size);

    if (num == 0)
        buffer[w--] = '0';

    buffer[BUFSIZ - 1] = '\0';

    while (num > 0)
    {
        buffer[w--] = map_to[num % 16];
        num /= 16;
    }

    if (flags & F_HASH && init_num != 0)
    {
        buffer[w--] = flag_ch;
        buffer[w--] = '0';
    }

    w++;

    return (write_unsgnd(0, w, buffer, flags, wid, prec, size));
}

